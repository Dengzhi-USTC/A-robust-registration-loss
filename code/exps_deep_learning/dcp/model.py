...
def attention(query, key, value, mask=None, dropout=None):
    ...
    scores = torch.matmul(query,
                          key.transpose(-2, -1).contiguous()) / math.sqrt(d_k)
    if mask is not None:
...
def nearest_neighbor(src, dst):
    inner = -2 * torch.matmul(src.transpose(1, 0).contiguous(),
                              dst)  # src, dst (num_dims, num_points)
    distances = -torch.sum(src**2, dim=0, keepdim=True).transpose(
        1, 0).contiguous() - inner - torch.sum(dst**2, dim=0, keepdim=True)
...
class Generator(nn.Module):
    def __init__(self, emb_dims):
        ...
        #                         nn.BatchNorm1d(emb_dims // 2), nn.ReLU(),
        #                         nn.Linear(emb_dims // 2, emb_dims // 4),
        #                         nn.BatchNorm1d(emb_dims // 4), nn.ReLU(),
        #                         nn.Linear(emb_dims // 4, emb_dims // 8),
        #                         nn.BatchNorm1d(emb_dims // 8), nn.ReLU())
        self.nn = nn.Sequential(nn.Linear(emb_dims, emb_dims // 2),
                                nn.GroupNorm(8, emb_dims // 2), nn.ReLU(),
                                nn.Linear(emb_dims // 2, emb_dims // 4),
                                nn.GroupNorm(8, emb_dims // 4), nn.ReLU(),
                                nn.Linear(emb_dims // 4, emb_dims // 8),
                                nn.GroupNorm(8, emb_dims // 8), nn.ReLU())
...
class PositionwiseFeedForward(nn.Module):
    ...
    def forward(self, x):
        return self.w_2(
            self.norm(F.relu(self.w_1(x)).transpose(2,
                                                    1).contiguous()).transpose(
                                                        2, 1).contiguous())
...
class PointNet(nn.Module):
    def __init__(self, emb_dims=512):
        ...
        self.bn4 = nn.GroupNorm(16, 128)
        self.bn5 = nn.GroupNorm(16, emb_dims)
...
class Transformer(nn.Module):
    def __init__(self, args):
        ...
        self.model = EncoderDecoder(
            Encoder(EncoderLayer(self.emb_dims, c(attn), c(ff), self.dropout),
                    self.N),
            Decoder(
                DecoderLayer(self.emb_dims, c(attn), c(attn), c(ff),
                             self.dropout), self.N), nn.Sequential(),
    ...
    def forward(self, *input):
        ...
        tgt_embedding = self.model(src, tgt, None,
                                   None).transpose(2, 1).contiguous()
        src_embedding = self.model(tgt, src, None,
                                   None).transpose(2, 1).contiguous()
        return src_embedding, tgt_embedding


# generate the corrspondence for src and tar?
class SVDHead(nn.Module):
    ...
    def forward(self, *input):
        ...
        H = torch.matmul(src_centered,
                         src_corr_centered.transpose(2, 1).contiguous())
...
#　Don't have any iterate , just use the Straight predicted！
