...
def print_info(opt, log_dir=None):
    ...
    #     git_date = datetime.fromtimestamp(
    #         repo.head.object.committed_date).strftime('%Y-%m-%d')
    #     git_message = repo.head.object.message
    #     _logger.info('Source is from Commit {} ({}): {}'.format(
    ...
    if log_path is None:
        if opt.dev:
            ...
            if opt.name is not None:
                log_path = os.path.join(opt.logdir,
                                        datetime_str + '_' + opt.name)
            else:
