...
def match_features(feat_src, feat_ref, metric='l2'):
    ...
    if metric == 'l2':
        ...
        feat_src_norm = feat_src / (
            torch.norm(feat_src, dim=-1, keepdim=True) + _EPS)
        feat_ref_norm = feat_ref / (
            torch.norm(feat_ref, dim=-1, keepdim=True) + _EPS)
    ...
    if slack:
        ...
        for i in range(n_iters):
            ...
            log_alpha_padded = torch.cat(
                (log_alpha_padded[:, :-1, :] - (torch.logsumexp(
            ...
            log_alpha_padded = torch.cat(
                (log_alpha_padded[:, :, :-1] - (torch.logsumexp(
                 ...
                 log_alpha_padded[:, :, -1,
                                  None]),  # Don't normalize last column
                dim=2)
...
def compute_rigid_transform(a: torch.Tensor, b: torch.Tensor,
...
class RPMNet(nn.Module):
    ...
    def compute_affinity(self, beta, feat_distance, alpha=0.5):
        ...
        if isinstance(alpha, float):
            ...
            hybrid_affinity = -beta[:, None, None] * (feat_distance -
                                                      alpha[:, None, None])
    ...
    def forward(self, data, num_iter: int = 1):
        ...
        xyz_ref, norm_ref = data['points_ref_sample'][:, :, :3], data[
            'normals_ref'][:, :, :3]
        xyz_src, norm_src = data['points_src_sample'][:, :, :3], data[
            'normals_src'][:, :, :3]
